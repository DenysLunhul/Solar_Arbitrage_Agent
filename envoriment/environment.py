...
class Environment(gym.Env):
    ...
    def __init__(self, df_raw: pd.DataFrame, df: pd.DataFrame, system_config: dict):
        ...
        # ── Батарея ───────────────────────────────────────────────
        # Всі параметри приходять з system_config — нічого не захардкоджено
        self.max_batt_capacity    = batt['capacity_kwh']
        self.max_batt_power       = batt['max_charge_power']
        ...
        # ── Мережа ───────────────────────────────────────────────
        self.max_grid_capacity    = inv['max_power']
        self.max_grid_capacity_ts = self.max_grid_capacity / 4
        self.price_to_buy = inv['price_to_buy']
    ...
    def step(self, action):
        ...
        # 5.1 Ринковий P&L
        if actual_grid_ts > 0:
    # Купуємо з мережі — платимо фіксований тариф клієнта
            reward -= actual_grid_ts * self.price_to_buy
        else:
    # Продаємо в мережу — отримуємо ринкову ціну DAM
        ...
        if unmet_load > 0:
            reward -= unmet_load * self.price_to_buy * 2
        ...
        if self.soc < self.soc_soft_min:
            soc_below = self.soc_soft_min - self.soc
            reward -= 3.0 * (soc_below ** 2)
 
        if self.soc > self.soc_soft_max:
            soc_above = self.soc - self.soc_soft_max
